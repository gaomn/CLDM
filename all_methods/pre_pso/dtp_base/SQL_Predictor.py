...
class Predictor:
    ...
    def train(self, t):
        ...
        if t < 4:
            return False
        x1_lst_ = [line[0] for line in self.Zprv]
        ...
        plt.savefig(self.args.fig_filename + f'/steps(b={self.args.time_fac})_{t}.jpg')
        plt.close()
        ...
        for s_num in range(2):
            ...
            # for i in range(len(Zprv)):
            #     tr = self.get_y(Zprv[i], 't')
            #     print(f'   tri   {tr}')
            #     print(tr[s_num])
    ...
    def dim_reduction(self, Zprv, Snxt, t):
        for j in range(2):
            ...
            for i in range(1 + self.x_dim):
                ...
                # print(f'x_shape: {x_list.shape}   y_shape: {np.array(y_list).shape}')
    ...
    def get_y(self, z, type):
        ...
        if type == 'Q' or type == 'q':
            ...
            for  s_num in range(2):
                ...
                for i in range(len(self.I[str(s_num)])):
                    for j in range(len(self.I[str(s_num)])):
                        Q_value[int(s_num)] += self.Quadratic_param[str(s_num)][int(1+i*len(self.I[str(s_num)])+j)]\
        ...
        # S型函数拟合
            ...
            for s_num in range(2):
                ...
                for i in range(len(self.I[str(s_num)])):
                    ...
                    S_value[s_num] += (1 / (1 + np.exp(self.alpha * datax[i]))-0.5) * self.Stype_param[str(s_num)][i+1]
    ...
    def check_pre(self):
        ...
        s_nxt = [self.predict([0., 0.], xi) for xi in rx]
        ...
        plt.xlabel(r'$x_1 Value$', fontsize=18)
        plt.ylabel('value', fontsize=18)
        plt.title(f'predict_fl step={t} -- (b={self.args.time_fac})', fontsize=18)
        plt.legend((s1, s2), (f"$s1_lst$", f"$s2_lst$"), loc='best')
        plt.savefig(self.args.fig_filename + f'/steps_fl(b={self.args.time_fac})_{t}.jpg')
        plt.close()

...
from utils.argument_tool import *
...
def draw_from_file(**kwargs):
    ...
    mean_lst3, std_list3 = get_data_list([f'b={int(time_fac)}_MPB', '_OUR'], data_file, max_step)
    ...
    b_min = np.min([np.min(mean_lst1) - 0.1 * np.max(mean_lst1), np.min(mean_lst2) - 0.1 * np.max(mean_lst2),
                    np.min(mean_lst3) - 0.1 * np.max(mean_lst3)])
    b_max = np.max([np.max(mean_lst1) + 0.1 * np.max(mean_lst1), np.max(mean_lst2) + 0.1 * np.max(mean_lst2),
                    np.max(mean_lst3) + 0.1 * np.max(mean_lst3)])
    ax.axis([0, max_step, b_min, b_max])
    ...
    data_dict = {'title': title,
                 'bt-type': xlabel[:3],
                 'time_fac': int(time_fac),
                 'Our method': f'{round(mean_lst3[-1])} \u00B1 {round(std_list3[-1])}',
                 'Optimal': f'{round(mean_lst1[-1])} \u00B1 {round(std_list1[-1])}',
                 'PSO only': f'{round(mean_lst2[-1])} \u00B1 {round(std_list2[-1])}'}
    return data_dict

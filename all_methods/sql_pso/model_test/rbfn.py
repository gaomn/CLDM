...
class RBFN:
    ...
    def _calc_sigmas(self):
        ...
        """
        # c_ = np.expand_dims(self.centers, 1).astype(np.float16)
        # ds = self.norm_func(c_ - self.centers).astype(np.float16)
    ...
    def _calc_interpolation_mat(self, X):
        ...
        """
        # x = np.expand_dims(X, 1).astype(np.float16)
        # r = self.norm_func(x - self.centers).astype(np.float16)
    ...
    def fit(self, X, y):
        ...
        # X_ = np.c_[np.ones(len(tmp)), tmp].astype(np.float16)
        # y = y.reshape((-1, 1))
